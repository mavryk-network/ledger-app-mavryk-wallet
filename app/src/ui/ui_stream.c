...
//! Init array consists of MV_SCREEN_LINES + background + left & right arrow +
//! picture.
#define UI_INIT_ARRAY_LEN (4 + MV_SCREEN_LINES_11PX)
...
static const char  *find_icon(mv_ui_icon_t icon);
   ...
   __attribute__((unused)) unsigned int button_mask_counter)
{
    ...
    case BUTTON_EVT_RELEASED | BUTTON_LEFT | BUTTON_RIGHT:
        ...
        if (cb_type & MV_UI_STREAM_CB_MAINMASK) {
            global.step = ST_IDLE;
            ui_home_init();
        }
        break;
...
static const char *
find_icon(mv_ui_icon_t icon)
{
    // clang-format off
    switch (icon) {
    case MV_UI_ICON_TICK:       return (const char *)&C_icon_validate_14;
    case MV_UI_ICON_CROSS:      return (const char *)&C_icon_crossmark;
    case MV_UI_ICON_DASHBOARD:  return (const char *)&C_icon_dashboard_x;
    case MV_UI_ICON_SETTINGS:   return (const char *)&C_icon_coggle;
    case MV_UI_ICON_BACK:       return (const char *)&C_icon_back_x;
    case MV_UI_ICON_EYE:        return (const char *)&C_icon_eye;
    case MV_UI_ICON_WARNING:    return (const char *)&C_icon_warning;
    default:                    return NULL;
    }
    // clang-format on
}
...
display_init(bagl_element_t init[UI_INIT_ARRAY_LEN])
{
    ...
    if (s->current > 0) {
        /* Unless we can't... */
        if (s->current == (s->total - MV_UI_STREAM_HISTORY_SCREENS + 1)) {
 ...
 */
static void
redisplay_screen(mv_ui_layout_type_t layout, uint8_t icon_pos)
{
    MV_PREAMBLE(("void"));
    mv_ui_stream_t *s = &G_stream;
    size_t          bucket;
    bucket                     = s->current % MV_UI_STREAM_HISTORY_SCREENS;
    mv_ui_icon_t icon          = s->screens[bucket].icon;
    bool         has_home_mask = layout & MV_UI_LAYOUT_HOME_MASK;
    layout                     = layout & ~MV_UI_LAYOUT_HOME_MASK;
  ...
  //   fill, fgcolor, bgcolor, font_id, icon_id}, text/icon
    ...
    if (has_home_mask) {
        ...
        // as icon_pos = txt_start_line in MV_UI_LAYOUT_PB layout,
                   ...
                   mv_ui_layout_type_t layout_type, mv_ui_icon_t icon)
{
    ...
    s->total++;
    int bucket = s->total % MV_UI_STREAM_HISTORY_SCREENS;
    ...
    if ((s->total > 0)
        && (bucket == (s->last % MV_UI_STREAM_HISTORY_SCREENS))) {
        drop_last_screen();
    }
    ...
    short line = 0;
    while ((offset < length) && (line < MV_UI_STREAM_CONTENTS_LINES)) {
        uint8_t will_fit;
        ...
        will_fit = mv_ui_max_line_chars(&value[offset], length - offset);
    ...
    for (line = 0; line < MV_UI_STREAM_CONTENTS_LINES; line++) {
...
drop_last_screen(void)
{
    ...
    if (s->screens[bucket].title) {
        MV_CHECK(ui_strings_drop(&s->screens[bucket].title));
    }
    for (i = 0; i < MV_UI_STREAM_CONTENTS_LINES; i++) {
        if (s->screens[bucket].body[i]) {
            MV_CHECK(ui_strings_drop(&s->screens[bucket].body[i]));
        }
    }
    s->last++;
